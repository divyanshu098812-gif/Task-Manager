...
def inject_sidebar_data():
    if current_user.is_authenticated:
        try:
            ...
            cursor.execute("SELECT COUNT(*) FROM notifications WHERE user_id=%s AND is_read=0", (current_user.id,))
...
def create_notification(user_id: int, notif_type: str, title: str, message: str = '') -> None:
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO notifications (user_id, type, title, message) VALUES (%s, %s, %s, %s)", (user_id, notif_type, title, message))
        conn.commit()
        conn.close()
    except Exception:
        pass
    ...
    try:
        ...
        for t in tasks_list:
            if t['status'] != 'Completed' and t.get('deadline') and t['deadline'] < today:
                cursor.execute("SELECT id FROM notifications WHERE user_id=%s AND type='overdue' AND message LIKE %s AND created_at >= NOW() - INTERVAL 1 DAY", (current_user.id, f"%#{t['id']}%"))
                if not cursor.fetchone():
                    create_notification(current_user.id, 'overdue', f'🔥 Overdue: {t["title"]}', f'This task is past its deadline. #{t["id"]}')

        for t in tasks_list:
            if t['status'] != 'Completed' and t.get('deadline') and t['deadline'] == today:
                cursor.execute("SELECT id FROM notifications WHERE user_id=%s AND type='due_today' AND message LIKE %s AND created_at >= CURDATE()", (current_user.id, f"%#{t['id']}%"))
                if not cursor.fetchone():
                    create_notification(current_user.id, 'due_today', f'📅 Due Today: {t["title"]}', f'This task is due today. #{t["id"]}')
...
def signup():
    if request.method == 'POST':
        ...
        if len(username) < 3:
            return render_template('signup.html', error='Username must be at least 3 characters.', form_data={'username': username, 'email': email})

        if password != confirm_password:
            return render_template('signup.html', error='Passwords do not match.', form_data={'username': username, 'email': email})

        if len(password) < 6:
            return render_template('signup.html', error='Password must be at least 6 characters.', form_data={'username': username, 'email': email})
        ...
        try:
            ...
            if cursor.fetchone():
                conn.close()
                return render_template('signup.html', error='That username is already taken.', form_data={'username': username, 'email': email})
            ...
            create_notification(new_id, 'welcome', f'👋 Welcome to Nexus Task, {username}!', 'Your account is ready. Start adding tasks to get productive!')

            return redirect(url_for('login', registered=1))

        except DBError as e:
            if e.errno == 1062:
                return render_template('signup.html', error='An account with that email already exists.', form_data={'username': username})
...
def add_task():
    if request.method == 'POST':
        ...
        try:
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("INSERT INTO tasks (title, description, deadline, user_id) VALUES (%s, %s, %s, %s)", (title, description, deadline, current_user.id))
            conn.commit()
            conn.close()
...
def delete_task(id):
    try:
        ...
        if task:
            create_notification(current_user.id, 'task_deleted', f'🗑 Task Deleted: {task["title"]}', 'The task has been permanently removed.')
...
def toggle_status(id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT title, status FROM tasks WHERE id=%s AND user_id=%s", (id, current_user.id))
        task = cursor.fetchone()
        if not task:
            conn.close()
        ...
        if new_status == 'Completed':
            create_notification(current_user.id, 'task_completed', f'🏆 Task Completed: {task["title"]}', 'Great work! Keep up the momentum.')
        else:
            create_notification(current_user.id, 'task_pending', f'↩ Task Reopened: {task["title"]}', 'Task has been moved back to pending.')
...
def edit_task(id):
    ...
    if request.method == 'POST':
        ...
        try:
            cursor.execute("UPDATE tasks SET title=%s, description=%s, deadline=%s WHERE id=%s AND user_id=%s", (title, description, deadline, id, current_user.id))
            conn.commit()
            conn.close()
...
def analytics():
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT * FROM tasks WHERE user_id=%s ORDER BY created_at DESC", (current_user.id,))
...
def calendar_view():
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT id, title, deadline, status FROM tasks WHERE user_id=%s", (current_user.id,))
        tasks = cursor.fetchall()
        conn.close()
        for t in tasks:
            if t.get('deadline') and hasattr(t['deadline'], 'strftime'):
...
def api_get_notifications():
    try:
        conn = get_db_connection()
        cursor = conn.cursor(dictionary=True)
        cursor.execute("SELECT id, type, title, message, is_read, DATE_FORMAT(created_at, '%%Y-%%m-%%d %%H:%%i:%%s') AS created_at FROM notifications WHERE user_id=%s ORDER BY created_at DESC LIMIT 50", (current_user.id,))
        notifications = cursor.fetchall()
        cursor.execute("SELECT COUNT(*) AS cnt FROM notifications WHERE user_id=%s AND is_read=0", (current_user.id,))
